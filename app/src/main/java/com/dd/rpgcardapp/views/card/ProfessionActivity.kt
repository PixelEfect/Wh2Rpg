...
import com.dd.rpgcardapp.data.ProfessionDescriptions
...
import com.dd.rpgcardapp.views.ProfessionRepository
...
class ProfessionActivity : BaseActivity() {
    ...
    fun updateProfessionStats(profession: Profession) {
        ...
        val quoteTextView = findViewById<TextView>(R.id.inputQuote)
        val descriptionTextView = findViewById<TextView>(R.id.inputDescription)
        ...
        val descriptionText = ProfessionDescriptions.descriptions[profession]

        // Jeśli istnieją ścieżki dla tej profesji
        if (descriptionText != null) {
            // Łączenie ścieżek wejściowych i ustawianie w TextView
            val quote = descriptionText.quote
            quoteTextView.text = quote.ifEmpty { "" }

            // Łączenie ścieżek wyjściowych i ustawianie w TextView
            val description = "Opis profesji: " + descriptionText.description
            descriptionTextView.text = description.ifEmpty { "" }
        } else {
            quoteTextView.text = ""
                    ...
                    if (race != null) {
                        ...
                        professionsTextView.setOnClickListener {
                            ...
                            showAlertDialog(
                                ...
                                selectedProfession =
                                    race.startingProfessions.first { it.name == selectedProfessionName }
                            ...
                            if (professionNames.isNotEmpty()) {
                                ...
                                professionsTextView.setOnClickListener {
                                    showAlertDialog(
                                        ...
                                        val selected =
                                            Professions.professionMap[selectedProfessionName]
                                        if (selected != null) {
                                            selectedProfession = selected
                                            professionsTextView.text = selected.name
                                            updateProfessionStats(selected)
